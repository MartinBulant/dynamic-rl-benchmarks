...
class PlatformsEnv(GoalEnv):
        ...
        def update(self, agent_pos, delta_t):
            ...
            if not self.is_active:
                if (
                    ...
                    < agent_pos[0]
                    < self.pos[0] + 0.5 * self.width
                    and self.pos[1] - 0.5 * self.height
                    < agent_pos[1]
                    < self.pos[1] + 0.5 * self.height
                ):
                    self.is_active = True
                    self.time_since_act = 0.0
    ...
    def _create_kinematic_b2_rect(self, pos_func, vel_func, width, height, color):
        body = self.world.CreateKinematicBody(
            position=(0.0, 0.0), shapes=b2PolygonShape(box=(0.5 * width, 0.5 * height))
        )
    ...
    def __init__(self, max_episode_length=500, subgoal_radius=0.05):
        ...
        self._subgoal_acs = [
            ArrowConfig(
                self._velocity_scale, self._vector_width, self._arrow_head_size, color
            )
        ...
        self._create_static_b2_rect(
            -0.0, -0.5 * self.level_width - 0.5, self.level_width, 1.0
        )
        self._create_static_b2_rect(-0.25, ground_level - 0.35, 1.5, 0.7)  # ground
        self._create_static_b2_rect(
            -0.5 * self.level_width - 0.5, 0.0, 1.0, 10
        )  # left wall
        self._create_static_b2_rect(
            0.5 * self.level_width + 0.5, 0.0, 1.0, 10
        )  # right wall
        ...
        level_thickness = 0.1
        self._create_static_b2_rect(
            -0.5, self.level_2 - 0.5 * level_thickness, 1.0, level_thickness
        )  # level 1
        ...
        self._create_kinematic_b2_rect(
            pos_func=lambda t: (
                0.75,
                0.5 * (ground_level + level_1)
                - 0.5 * level_thickness
                + 0.5 * diff1 * np.sin(omega1 * t + phi1),
            ),
            vel_func=lambda t: (0.0, omega1 * 0.5 * diff1 * np.cos(omega1 * t + phi1)),
            width=0.5,
        ...
        self._create_kinematic_b2_rect(
            pos_func=lambda t: (
                ...
                * diff1
                * np.cos(
                    omega2 * self.triggers[0].time_since_act * self.time_step + phi2
                ),
            ),
            vel_func=lambda t: (
                (
        ...
        self.max_vel_comp = 1
        self.max_ang_vel = (
            2 * np.pi * self.max_vel_comp / (2 * np.pi * self.agent_radius)
        )
        ...
        self.ball = self.world.CreateDynamicBody(
            position=self.agent_initial_position, fixtures=ball_fixture
        )
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        if (
            np.linalg.norm(achieved_goal - desired_goal)
    ...
    def render(self, mode="human", close=False):
        ...
        def draw_timed_circular_subgoal(
            position, velocity, delta_t_ach, delta_t_comm, radius, color, arrow_config
        ):
            ...
            draw_box(
                position + (0.0, radius + 0.04), delta_t_ach / 100.0, 0.03, 0.0, color
            )
            # subgoal
        ...
        # goal
        draw_circle_sector(
            self.goal, 0.0, self.goal_radius, n_triangles, self.goal_color, n_triangles
        )
        draw_circle_sector(
            self.goal,
            0.0,
        ...
        # subgoals
        for sg, color, ac in zip(
            self._subgoals, self._subgoal_colors, self._subgoal_acs
        ):
            if sg is not None:
                draw_circular_subgoal(
                    sg["position"],
        ...
        for ts, color, ac, tol in zip(
            ...
            if ts is not None:
                ...
                draw_timed_circular_subgoal(
                    ts.goal["position"],
                    ts.goal["velocity"] * self.max_vel_comp,
                    ts.delta_t_ach,
                    ts.delta_t_comm,
                    r,
                    color,
        ...
        draw_line(
            self.ball.position, self.agent_radius, self.ball.angle, self.line_color
        )
